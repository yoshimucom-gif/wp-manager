...
- 見出し（H2/H3）の SEO 品質基準（重要）:
  ...
  【ステップ1: 主要キーワードの抽出】
  記事タイトルから「核となる短い語」を 1〜3 個抽出してください。
  - 抽出対象: **商品カテゴリ・対象物・状態・課題**を表す名詞中心の語
  - 除外: 「おすすめ」「ベスト」「人気」「厳選」「商品N選」「N選」のような
    評価語・順序語・数詞は主要キーワードに含めない
  例: タイトル「キャスターチェアによる床の傷を防止するおすすめ商品5選」
      → 主要キーワード候補: 「キャスターチェア」「床傷防止」「床保護」「キャスターストッパー」
      → 除外: 「おすすめ」「5選」

  【ステップ2: H2 見出しの作り方】
  検索エンジンと読者の両方に「そのセクションが何の話か」を理解させる
  **ちょうど良い具体性**が必要。両極端は避ける:

    ❌ 過剰（タイトル丸ごとコピペ、｜区切りで3要素以上、同義語重複）
       例: 「キャスターチェアによる床の傷を防止する｜選定基準｜この5製品を選んだ理由」
       理由: 読者には冗長、Google には過剰最適化と判定される

    ❌ 不足（単語1つだけの裸見出し、共起語ゼロ）
       例: 「選定基準」「よくある質問」「まとめ」だけ
       理由: そのH2セクションが**何の**選定基準か Google に伝わらず、
       E-E-A-T／トピカルオーソリティで弱い

    ✅ 最適（主要キーワードを 1 語、自然な日本語として組み込む。
       文字数目安は 12〜30 文字。｜は最大1個）
       例（主要キーワード「キャスターチェア」「床傷防止」を使う場合）:
         「キャスターチェアで床に傷がつく原因と対策」
         「床傷防止グッズの選定基準｜4つの評価軸」
         「キャスターチェア用 床保護グッズの選び方」
         「失敗しない床傷防止アイテムの選び方」
         「キャスターチェアの床傷対策によくある質問」
         「まとめ｜床傷防止グッズ選びで失敗しないために」

  【ステップ3: H2 を書くときのルール】
  1. 各 H2 に主要キーワードを 1 語、自然な日本語として組み込む
  2. **タイトル全文や 10 文字以上の連続フレーズ**の貼り付けは絶対にしない
  3. 「○○による」「○○のための」「○○を○○する」のような中途半端な
     助詞節止まりの見出しも避ける（自然な日本語として完結させる）
  4. 「｜」記号は使うとしても 1 見出しに最大 1 個まで（記事全体で 2〜3 個まで）
  ...
  任意で含める要素:
  - 識別語 1 個まで（「5個セット」「黒」「Pro」など、本質的な区別に必要なもの）

  含めない要素:
  - キャッチコピー（「毛玉がつかない」「家具が楽に移動できる」）
  - 効能語（「キズ防止」「振動吸収」「滑り止め」）
  - 用途語（「フローリング用」「オフィス用」「賃貸対応」）
  - タイトル丸ごとの貼り付け

  文字数目安: 15〜30 文字。Amazon の商品名そのままを貼り付けるのは禁止。

    ❌ 「1位：Ezprotekt キャスターストッパー 5個セット 毛玉がつかない
        キャスター 椅子 固定 傷防止 家具が楽に移動できる 振動吸収
        家具保護パッド キズ防止 フローリング用」
       理由: 長すぎて Amazon API の関連性スコアが下がる + 読者が識別困難
    ❌ 「1位：商品名｜キャスターチェアによる床の傷を防止する」（H3 末尾にタイトル）
    ❌ 「1位：キャスターストッパー」（ブランド名なし → 別ブランド大量混入）
    ❌ 「1位：ダイソン」（カテゴリなし → 関係ない掃除機・扇風機まで混入）

    ✅ 「1位：Ezprotekt キャスターストッパー（5個セット）」
    ✅ 「1位：Ezprotekt キャスターストッパー」
    ✅ 「1位：ダイソン V15 Detect」（型番が強い識別子）
    ✅ 「1位：山善 オフィスチェア YHC-300」
    ✅ 「1位：サムコス キャスターストッパー 高密度フェルト」
...
- 文章の冗長を避ける（SEO Helpful Content 観点・重要）:
  ...
  ❌ 同じフレーズの繰り返し
     - 「対応キャスター径は公式情報でご確認ください」を商品ごとに10回繰り返さない
     - 「振動吸収機能も持ち合わせており」「5個セットで5本脚チェアに対応」など
       同じ評価フレーズを複数商品で使い回さない
     - 同義の文を別の言い回しで2回書かない（「○○です。つまり○○ということです」）

  ❌ 責任回避フレーズの濫用
     - 「公式情報をご確認ください」「公式サイトで要確認」「ご確認ください」を
       記事全体で**3回まで**に抑える。各商品の末尾に毎回貼らない
  ...
  ❌ 「向いている人 ul」「向いていない人 ul」の濫用
     - 商品10件すべてに同じ構造の ul を貼ると情報密度が下がる
     - 後述のランキング密度勾配ルールを優先する

  ✅ 各セクションは「他のセクションと内容が重複しない」ことを優先
  ✅ 同じ商品でも「ここでしか言えない独自の特徴」を 1〜2 個明示する
  ✅ 「○○のため」「○○なので」のような理由語を多用するより、
     具体的な数値・素材名・型番で差別化する
...
def trim_orphan_particles_from_heading_start(html):
    """見出しの冒頭・末尾にタイトル除去で取り残された不自然なフレーズを整理する。

    対象パターン:
    1. 冒頭の単独助詞「で／の／に／を／が／は／と」+ 漢字/カタカナ
       例「で床に傷がつく前に対策しよう」→「床に傷がつく前に対策しよう」
    2. 冒頭の区切り記号「｜選定基準」→「選定基準」
    3. 「の」が連続したケース「グッズのの選び方」→「グッズの選び方」
    4. 連続した区切り記号「｜｜」→「｜」
    5. 末尾の区切り取り残し「○○｜」→「○○」
    """
    if not html:
    ...
    def fix(m):
        ...
        # (1) 冒頭の単独助詞 + 漢字/カタカナ（実体語）の場合だけ削除
        new_inner = re.sub(r'^[\s　]*[でをにがはとの](?=[一-龥ァ-ヶー])', '', new_inner)
        # (2) 冒頭の区切り記号を整理
        new_inner = re.sub(r'^[\s　]*[｜|：:・\-―—][\s　]*', '', new_inner)
        # (3) 「の」が連続「のの」→「の」
        new_inner = re.sub(r'のの+', 'の', new_inner)
        # (4) 連続した区切り記号「｜｜」→「｜」
...
def detect_heading_quality_issues(title, html):
    ...
    # 閾値を 12 にして、「キャスターチェア」「床傷防止」などの主要キーワード
    # （見出しに含めるべきSEO要素）を誤って「タイトル丸ごとコピペ」と判定
    # しないようにする。sanitize 側の strip_title_substring と整合させる。
    title_dup_count = 0
    if len(norm_title) >= 12:
        # 12文字以上の連続部分文字列を candidates にする
        candidates = set()
        for length in range(min(len(norm_title), 30), 11, -1):
    ...
    # (6) 裸見出し（共起語ゼロ）の検出
    # 「選定基準」「よくある質問」「まとめ」のように主要キーワードを一切含まない
    # 短い見出しが多いと SEO 的に弱い。文字数 8 未満かつ
    # タイトル由来の主要キーワード（4文字以上の連続漢字/カタカナ）を全く含まない
    # H2 が 3 個以上あれば減点する。
    h2_only_pattern = re.compile(r'<h2[^>]*>(.*?)</h2>', re.IGNORECASE | re.DOTALL)
    h2_text_list = [re.sub(r'<[^>]+>', '', m).strip() for m in h2_only_pattern.findall(html)]
    title_main_terms = re.findall(r'[一-龥]{3,}|[ァ-ヶー]{3,}', norm_title)
    bare_h2_count = 0
    for h2t in h2_text_list:
        if len(h2t) >= 8:
            continue
        # 主要キーワード（タイトルから抽出した3文字以上の語）を含むかチェック
        has_kw = any(t in h2t for t in title_main_terms) if title_main_terms else False
        if not has_kw:
            bare_h2_count += 1
    if bare_h2_count >= 3:
        penalty += 10
        cap = min(cap, 70) if cap is not None else 70
        issues.append(f'共起語のない裸見出し（「選定基準」「まとめ」など短くて主要キーワードを含まない見出し）が {bare_h2_count} 件あります。SEO的に弱いので、各見出しに主要キーワードを1語自然に含めてください。')

    # (7) H3 ランキング商品名の長さチェック
    # 「1位：」「2位：」のような順位付き H3 に着目し、極端に長い／短いものを検出。
    # ランキング系見出しの目安: 15〜50文字。
    h3_pattern = re.compile(r'<h3[^>]*>(.*?)</h3>', re.IGNORECASE | re.DOTALL)
    h3_text_list = [re.sub(r'<[^>]+>', '', m).strip() for m in h3_pattern.findall(html)]
    rank_h3_pattern = re.compile(r'^(?:第)?[0-9０-９]+\s*位[:：]', re.UNICODE)
    too_long_rank_h3 = 0
    too_short_rank_h3 = 0
    for h3t in h3_text_list:
        if not rank_h3_pattern.match(h3t):
            continue
        # 順位部分を除いた本体の文字数
        body = rank_h3_pattern.sub('', h3t, count=1).strip()
        body_len = len(body)
        if body_len > 50:
            too_long_rank_h3 += 1
        elif body_len < 6:
            too_short_rank_h3 += 1
    if too_long_rank_h3 > 0:
        penalty += min(15, too_long_rank_h3 * 4)
        cap = min(cap, 70) if cap is not None else 70
        issues.append(f'ランキング H3 の商品名が長すぎる見出しが {too_long_rank_h3} 件あります（Amazon商品名そのままの可能性）。「ブランド名＋商品カテゴリ＋識別語1個」程度の 15〜30 文字に絞ってください。商品検索の精度も下がります。')
    if too_short_rank_h3 > 0:
        penalty += too_short_rank_h3 * 3
        cap = min(cap, 75) if cap is not None else 75
        issues.append(f'ランキング H3 の商品名が短すぎる見出しが {too_short_rank_h3} 件あります（ブランド名やカテゴリが不足）。Amazon検索でヒット率が下がります。「ブランド名＋商品カテゴリ」を必ず含めてください。')

    # (8) 冗長検出: 責任回避フレーズの濫用
    # 「公式情報をご確認ください」「公式サイトで要確認」「ご確認ください」が
    # 記事全体で 10 回以上出ていたら水増しの可能性。8回以上で警告。
    bare_text = re.sub(r'<[^>]+>', '', html)
    confirm_phrase_count = (
        bare_text.count('公式情報')
        + bare_text.count('公式サイト')
        + bare_text.count('ご確認ください')
    )
    if confirm_phrase_count >= 15:
        penalty += 10
        cap = min(cap, 75) if cap is not None else 75
        issues.append(f'「公式情報を確認」「ご確認ください」のような責任回避フレーズが {confirm_phrase_count} 回登場しています。共通の注意点は「選定基準」「選び方」セクションで 1 回だけ書き、各商品で繰り返さないでください。')
    elif confirm_phrase_count >= 10:
        penalty += 5
        issues.append(f'「公式情報を確認」系のフレーズが {confirm_phrase_count} 回登場しています。SEO 的にノイズになるので回数を絞ってください。')

    # (9) 冗長検出: 同じフレーズの繰り返し
    # 15文字以上の連続フレーズが3回以上同一記事に出てくる場合に減点
    # 「5個セットで5本脚チェアに対応」のような評価フレーズの使い回しを検出
    phrase_repeats = 0
    # 段落単位で見て、長めの連続フレーズの重複を簡易検出
    chunks = re.findall(r'[一-龥ぁ-んァ-ヶー]{15,40}', bare_text)
    chunk_counter = {}
    for c in chunks:
        chunk_counter[c] = chunk_counter.get(c, 0) + 1
    for c, n in chunk_counter.items():
        if n >= 3:
            phrase_repeats += 1
    if phrase_repeats >= 3:
        penalty += min(15, phrase_repeats * 3)
        cap = min(cap, 70) if cap is not None else 70
        issues.append(f'同じフレーズ（15文字以上）が 3 回以上繰り返されている箇所が {phrase_repeats} 件あります。各商品の解説で評価フレーズを使い回さず、独自性を持たせてください。')

    # (10) 冗長検出: まとめ後の追記連鎖
    # 「<h2>まとめ」以降の <p> が異常に多い場合に減点
    matome_match = re.search(r'<h2[^>]*>[^<]*(?:まとめ|総括|結論)', html, re.IGNORECASE)
    if matome_match:
        after_matome = html[matome_match.start():]
        p_after = re.findall(r'<p\b[^>]*>(.*?)</p>', after_matome, re.IGNORECASE | re.DOTALL)
        # ul / table を含めない純粋な p の数
        if len(p_after) >= 8:
            penalty += 8
            cap = min(cap, 75) if cap is not None else 75
            issues.append(f'まとめセクション以降に <p> が {len(p_after)} 個並んでいます。まとめは要点整理＋CTA で 1 セクション内で完結させ、追記パラグラフを連ねないでください。')
...
- 「対応キャスター径の確認」「素材の摩耗」「公式情報での確認」のような
  **全商品に共通する注意点**は、商品個別の解説で繰り返さず
  **「選定基準」または「選び方」セクションで1回だけ**書く
- 個別商品の注意点は、その商品**特有のリスク**（例: 「4個セットなので5本脚チェアでは1個不足」）
  のみ書く。汎用的な注意は書かない

「向いている人 ul」の使い方:
- 全{count}件に同じ構造の ul を貼ると冗長になる。**上位3〜5商品まで**に絞り、
  6位以降は本文中で「○○な方に向いています」と1文で言及する程度で良い

まとめセクションの設計:
- 要点整理＋自然な CTA で**1セクション内で完結**させる
- 「商品を選んだあとは○○してください」「また、どの商品を選んだとしても」
...
def strip_title_substring_from_headings(html, title=None, min_length=12):
    ...
    ⚠️ min_length は 12 に設定している（旧 8 から引き上げ）。
    SEO 的には「キャスターチェア」「床傷防止」など 6〜10 文字程度の
    主要キーワードは見出しに残っていた方が良い（共起語として機能する）。
    削るのは「キャスターチェアによる床の傷を防止する」のような
    タイトル全文相当の貼り付けだけにする。
...
PLUGIN_DOWNLOADS = {
    ...
    'auto-inserter': {
        'file': 'affiros-auto-inserter-0.16.2.zip',
    ...
    'sale-hub': {
        'file': 'affiros-sale-hub-1.1.1.zip',
        'name': 'Affiros セールハブ',
        'version': '1.1.1',
    },
}
...
PLUGIN_UPDATE_META = {
    ...
    'sale-hub': {
        'plugin_basename': 'affiros-sale-hub/affiros-sale-hub.php',
        'tested':   '6.6',
        'requires': '5.8',
        'requires_php': '7.4',
        'author':   'Affiros',
