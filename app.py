...
- リード文は読者の悩みと記事の結論を簡潔に。長く書きすぎない（自然に2〜3段落で十分）。
...
- H2「選び方」: 素材・価格・用途・サイズ感など判断軸を整理。要点を簡潔に。
- FAQ: 3〜5問。質問ごとにH3見出しを使い、回答は簡潔に。だらだら書かない。
- **最後に必ずH2「まとめ」を入れて記事を完結させる**。読者の次の行動を示し、自然に締める。
- ランキング個別解説は繰り返さないでください。
- **完結性を最優先**: 選び方・FAQ・まとめの3セクションを全て入れて記事を終わらせる。
  途中で止めたり、文字数を気にして省略したりしない。冗長な繰り返しは避ける。"""
        })
        return steps
