...
class Decofmt_Validator {
    ...
    public static function validate($original, $decorated) {
        ...
        // 6. 既存ブロックの保持チェック（v1.0.18）
        //    dbp/voice（吹き出し）を dbp/frame（ボックス）に置き換える等、
        //    既存ブロックを別種に変換する事故を検出する。
        //    ここで error にすると decorator 側のリトライにエラー内容が
        //    フィードバックされ、モデルが自力で修正する。
        $block_errors = self::validate_preserved_blocks($original, $decorated);
        foreach ($block_errors as $err) {
            $errors[] = $err;
        }
    ...
    /**
     * 既存ブロックが装飾で減っていないか検証（v1.0.18）
     *
     * 装飾は「装飾を足す」処理なので、元にあったブロックの数が減ることは無いはず。
     * 減っている = 別種のブロックに置き換えられた（例: dbp/voice → dbp/frame）か
     * 削除されたということなので、エラーとして扱う。
     *
     * 監視対象は「置き換えられると困る」ブロックに絞る。
     * wp:paragraph / wp:heading は装飾で構造が変わる（分割・昇格）ため対象外。
     */
    private static function validate_preserved_blocks($original, $decorated) {
        $errors = [];

        // 監視するブロック名（前方一致）。ラベルは人間向けの説明。
        $watch = [
            'dbp/voice' => '吹き出し（dbp/voice）',
            'dbp/frame' => '囲みボックス（dbp/frame）',
            'image'     => '画像（wp:image）',
            'table'     => '表（wp:table）',
            'embed'     => '埋め込み（wp:embed）',
            'html'      => 'カスタムHTML（wp:html）',
            'shortcode' => 'ショートコード（wp:shortcode）',
        ];

        foreach ($watch as $block => $label) {
            $pattern = '/<!--\s*wp:' . preg_quote($block, '/') . '\b/i';
            $orig_n = preg_match_all($pattern, $original);
            $deco_n = preg_match_all($pattern, $decorated);
            if ($deco_n < $orig_n) {
                $errors[] = "既存の{$label}が減少（元:{$orig_n} / 装飾後:{$deco_n}）"
                    . "。既存ブロックを別の種類に置き換えたり削除したりしないでください";
            }
        }
