<?php
/**
 * Affiros プラグイン自動更新チェッカー
 *
 * Affiros9 サーバー(/api/plugin-update/<key>)をWordPress の更新サーバーとして使い、
 * 標準の「プラグイン更新」フローに組み込む。
 *
 * 動作:
 *   1. WP が定期的に pre_set_site_transient_update_plugins を叩く
 *   2. 本クラスが Affiros9 から JSON を取得し、ヘッダーバージョンと比較
...
if (!class_exists('Affiros_Plugin_Updater')) :

class Affiros_Plugin_Updater {
    /** プラグイン本体ファイル (__FILE__) */
    private $plugin_file;
    /** プラグインスラッグ (ディレクトリ名) */
    private $plugin_slug;
    /** プラグインベース名 (例: affiros-rewrite/affiros-rewrite.php) */
    private $plugin_basename;
    /** Affiros9 のアップデート情報エンドポイント URL */
    private $update_url;
    /** リモート情報キャッシュキー */
    private $cache_key;
    /** リモート情報キャッシュ TTL (秒) */
    ...
    public function __construct($plugin_file, $update_url, $cache_ttl = 21600) {
        ...
        $this->cache_key       = 'affiros_updater_' . md5($this->plugin_basename);
    ...
    /** Affiros9 サーバーから最新メタ情報を取得（キャッシュあり） */
    ...
    public function plugins_api_filter($result, $action, $args) {
        ...
        return (object)[
            ...
            'requires'     => isset($remote->requires) ? $remote->requires : '',
            'requires_php' => isset($remote->requires_php) ? $remote->requires_php : '',
            'author'       => isset($remote->author) ? $remote->author : 'Affiros',
