...
class Affiros_Rewrite_Article_Type {
    ...
    public static function infer($keyword = '', $title = '') {
        ...
        // 「○つの○○」パターンは column 寄り（ranking 判定の前にチェック）
        // 例: 4つの理由 / 5つのチェックポイント / 3つのコツ / 2つの注意点
        if (preg_match('/[0-9０-９]+\s*つ\s*の?\s*(?:チェック|ポイント|サイン|ステップ|理由|秘訣|コツ|心得|注意点|特徴|失敗|落とし穴|教訓|タイミング|目安|基準|メリット|デメリット|効果|条件|原則|ルール|ヒント|アドバイス|パターン|方法|手順|段階|タイプ|種類|違い)/u', $text)) {
            return 'column';
        }
