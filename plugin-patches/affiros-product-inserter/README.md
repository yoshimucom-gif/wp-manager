...
| 種別 | 内容 |
|---|---|
| ⭐ 新機能 | **カード／マーカー一括削除** を追加。期間（投稿日・挿入日）／カテゴリ／タグ／ステータス／デザイン種類で絞り込み、挿入済みカード・未処理マーカー・退避コメントをまとめて削除できる。「完全に削除」と「マーカーに戻す（入れ直せる状態）」の2択。削除前の本文は記事ごとに退避され1クリックで復元可能 |

v1.9.x 以前の変更履歴は [CHANGELOG.md](CHANGELOG.md) を参照してください。
...
- ✅ 一括処理（カテゴリ・タグで絞り込み）
- ✅ カード／マーカー一括削除（期間・カテゴリ・タグ・ステータス・種類で絞り込み。復元可）
...
│   ├── card-blocks.php           # カード/マーカーのブロック検出共通ヘルパー
...
│   ├── bulk-delete.php           # カード／マーカー一括削除
│   ├── adjacent-cards.php        # 連続カード／マーカー検出
