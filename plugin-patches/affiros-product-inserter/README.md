...
| 種別 | 内容 |
|---|---|
| ⭐ 新機能 | カードデザインに **`score`（スコアカード）** と **`mini`（ミニカード）** を追加。マーカー語彙には存在したがテンプレート未実装だった2デザインを実装 |
| 🐛 修正 | Amazonレビュー矛盾の解消 — Amazon PA-APIはレビューを返さない仕様のため、ペア成立した楽天商品のレビュー（評価・件数）を同一商品の代理値として引き継ぐように。AI選定プロンプトも「レビューデータなし＝低品質ではない」と明示 |
| 🐛 修正 | 再挿入・ロールバック時に「⚠️ 24h経過」フラグが残り続ける問題を修正 |
| 🐛 修正 | 一括処理のコスト試算が Claude Opus 4.7 に対応（旧キー `claude-opus-4-6` のままで効いていなかった） |
| 📝 整備 | README・デザインプレビュー画面を現状（6デザイン・2ボタン構成）に合わせて最新化 |
