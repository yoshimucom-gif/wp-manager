...
### 新機能: 公開時の自動商品挿入
- **背景**: 予約投稿が自動公開された後、毎日「商品挿入を実行」ボタンを手動で押す運用が面倒。
- **追加**:
  - `transition_post_status` フックで future→publish / draft→publish を検出。
  - マーカー入り（`<!--ai-product:...-->`）かつ未挿入の場合、WP Cron で N 分後（デフォルト 5 分）に非同期実行。
  - 手動の「商品挿入を実行」ボタンと**完全に同じ `AI_PI_Inserter::run()` を呼ぶ**ため、挿入品質は手動実行と同じ。独自ロジックを足していない。
  - 失敗時はマーカー残置 + post_meta `_ai_pi_auto_insert_last_error` にエラーを記録（既存挙動と同じ）。自動リトライなし。
  - プラグイン無効化時は `ai_pi_clear_auto_insert_crons()` で予約済み cron event を全て掃除。
- **設定UI**: 設定ページに「🤖 自動挿入」セクションを追加。
  - ON/OFF（既定 OFF）
  - 遅延実行（0〜60 分、既定 5 分）
- **新規ファイル**: `includes/auto-insert.php`
...
### 変更
- **比較表（compare）カードのヘッダー・価格・評価を削除。**
  「AI比較 おすすめ商品 比較表」ヘッダー、価格列、評価列を削除。
  APIから取得できない項目を空欄表示するのを廃止し、順位・商品・購入ボタンのみのシンプルな構成にした。
- **ランキング（ranking）カードのヘッダー・判断軸を削除。**
  「AI選定 この記事でおすすめの商品TOP〇」ヘッダーと「判断軸:」テキストを削除。
  シンプルな順位・商品・購入ボタン構成に統一した。
...
### 修正
- **compare/rankingのみのルールで商品が挿入されないバグを修正。**
  単体マーカー（vertical）が0件でcompare/rankingだけの場合、`article_products`が空になり
  商品カードが何も描画されない問題を修正。この場合はランキング選定で直接商品プールを構築する。
  コラム記事で比較表カードだけを設定した場合などに影響していた。
...
### 修正
- **再挿入時のマーカー未検出エラーを修正。** `inserter.php` でバックアップ優先ロジックを改善。
  Affiros9 から記事を再投稿した際、`post_content` が新しいマーカー入りコンテンツに
  上書きされているにもかかわらず、古いバックアップ（マーカーなし）を読んで
  「記事本文にマーカーが見つかりません」エラーになる問題を修正。
  - `post_content` にマーカーが存在する場合は「Affiros9 再投稿後の新規状態」と判断し
    `post_content` を優先、既存の `_ai_pi_inserted` / `_ai_pi_backup` フラグをリセットする。
  - マーカーなしの通常再挿入（カード描画済みのpost_contentからバックアップを読む）は従来通り。
