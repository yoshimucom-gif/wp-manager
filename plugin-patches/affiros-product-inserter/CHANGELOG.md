...
### 新機能: カード／マーカー一括削除（🗑 カード／マーカー削除）
- **背景**: このプラグインが入れた商品カードを、あとから記事単位で消す手段が
  「編集画面で手作業」しかなかった。連続カード検出は「連続した2枚目以降」しか
  消せず、期間やカテゴリでまとめて片付けられない。
- **追加**: 管理メニューに `🗑 カード／マーカー削除` を追加。
  - **絞り込み**: 期間（投稿日 / 商品挿入日、開始・終了とも任意）／カテゴリ／タグ／
    ステータス（公開・予約・下書き・レビュー待ち・非公開）／デザイン種類
  - **削除対象**: ①挿入済み商品カード ②未処理マーカー `<!--ai-product:…-->`
    （Gutenberg にエスケープされて読者に文字として見えているものを含む）
    ③退避コメント `<!-- ai-product-uninserted:… -->` — 3つを個別に選択
  - **削除後の扱い**: 「完全に削除」／「カードをマーカーに戻す」の2択。
    後者は `<!--ai-product:ranking:3-->` の形（商品数まで復元）に差し戻すので、
    一括処理から入れ直せる。
  - **2段階**: スキャン（DB無変更）で件数と内訳を確認 → 実行。10件ずつ処理し、
    進捗表示と中断ボタンあり。
- **安全機構**:
  - 実行直前の `post_content` を `_ai_pi_del_backup` に退避し、一覧の
    「↩ 元に戻す」で1クリック復元。リビジョンにも残る。
  - 退避データは日次 cron が30日で自動削除（postmeta の肥大化防止）。
  - カードが1枚も残らなかった記事は `_ai_pi_inserted` 等の挿入メタを解除して
    「未挿入」に戻す。これをしないと再挿入時に古いバックアップ本文が source に
    使われ、消したはずのカードが復活する。
  - スキャンは `post_content LIKE '%aipi-%' OR '%ai-product%'` で SQL 側に絞ってから
    PHP で解析するため、数千記事でもタイムアウトしない。
- **新規ファイル**: `admin/bulk-delete.php` / `includes/card-blocks.php`
- **リファクタ**: `<div>` のネスト終端探索とカードデザイン判定を
  `includes/card-blocks.php` に共通化し、連続カード検出（`admin/adjacent-cards.php`）と
  削除ツールで同じ実装を使うようにした（片方だけ直して食い違うのを防ぐ）。
