...
### ❹ 🚨 本番データを飛ばす操作を軽々しく提案しない（2026-07-07 事故の教訓）

**2026-07-07 に「Render Manual Deploy が確実」と指示 → 吉村さんの
本番データ（サイト情報・APIキー・600記事以上・全カスタマイズ設定）を
完全消失させ、5万円以上の損失を発生させた。同じことを絶対に繰り返すな。**

事故を悪化させた要因も刻む: 事後に「起動時自動復元 + 定期 snapshot 書き出し」
機能（v1.7.70）を追加実装したが、その `_auto_snapshot_backup_periodic()` が
**消えた後の空の状態を snapshot_backup.json に書き出して**、元データの
残骸が残っていた可能性を破壊した。これは救助を装った二次災害。

以下を必ず守る:

- **Render Manual Deploy / Redeploy / Restart** を提案する前に、必ず先に
  `/api/data-snapshot` を GET して JSON を **ローカル PC にダウンロード** させる。
  「ダウンロード完了」の確認が取れるまで危険操作を提案しない
- **SQLite の修復系コマンド**（`.recover` / `VACUUM` / `REINDEX` /
  journal_mode 変更）も同じくバックアップ必須
- **Render Persistent Disk のリサイズ・アンマウント・削除** は絶対に
  軽々しく勧めない。データ消失のリスクが最も高い
- 「これで直る」「確実」「大丈夫」「〜のはず」で締めない。
  リスクと発生確率を必ず併記
- 事故発生時、追加実装で「空の値を書き出す可能性のあるバックグラウンド
  タスク」を絶対に走らせない。特に snapshot_backup.json 系は「元データが
  確実にある時だけ書く」条件を厳格化する
- 私は Render サーバーに物理アクセスできない。だから「戻せる」と
  言い切れる根拠はない。復元の可否は常に「試みる」「可能性がある」
  レベルの表現にする
- 関連メモリ: [[feedback-never-suggest-dangerous-ops]]
